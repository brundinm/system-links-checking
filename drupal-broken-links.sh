...
# Purpose: Shell script to process Drupal content to identify any broken links

# Description: Bash shell script to run LinkChecker to process the Drupal URLs
...
# (2) The Drupal broken links tab-delimited file "drupal-broken-links.csv" from
...
echo -e "Beginning process to get Drupal broken links by conducting LinkChecker crawls . . .\n"

# Use the LinkChecker link validator to crawl Drupal using the seed URL of
# https://library.macewan.ca/ to identify any broken links on the site and
# write out to the CSV file "broken-links.csv"
linkchecker --no-robots --no-warnings --check-extern -o csv --user-agent "User-Agent: LinkChecker: MacEwan University Library links checking operation" --ignore-url ^data:image\/.*$ --ignore-url ^http:\/\/creativecommons\.org\/.*^ --ignore-url ^https://creativecommons.org\/.*$ --ignore-url ^mailto:.*$ --ignore-url ^tel:.*$ https://library.macewan.ca/ > broken-links.csv
...
gawk -v FPAT='"[^"]*"|[^;]*' -v OFS='\t' '{$1=$1} 1' broken-links.csv > drupal-broken-links.csv

# Remove data processing file
rm -f broken-links.csv

# Print processing statement
echo -e '\nFinished creating a tab-delimited file of Drupal broken links called "drupal-broken-links.csv"\n'

# Print final processing statement
echo 'The Drupal broken links processing operation is now finished!'
